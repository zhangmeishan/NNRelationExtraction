...
struct COutput {
  ...
  COutput() : in(NULL), bGold(0), ac(){
  }

  COutput(const COutput &other) : in(other.in), bGold(other.bGold), ac(other.ac) {
  }
};
...
public:
  ...
  inline int forward(Instance &inst, const vector<CAction> *goldAC = NULL) {
