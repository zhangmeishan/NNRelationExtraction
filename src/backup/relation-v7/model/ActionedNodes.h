...
struct ActionedNodes {
    ...
    PSubNode b2t_ancestor_exclude_nodei;
    PSubNode b2t_ancestor_exclude_nodej;

    PSubNode t2b_ancestor_exclude_nodei;
    PSubNode t2b_ancestor_exclude_nodej;
...
public:
	inline void initial(ModelParams &params, HyperParams &hyparams, AlignedMemoryPool *mem) {
        ...
        b2t_ancestor_exclude_nodei.init(hyparams.tree_lstm_dim, -1, mem);
        b2t_ancestor_exclude_nodej.init(hyparams.tree_lstm_dim, -1, mem);
        t2b_ancestor_exclude_nodei.init(hyparams.tree_lstm_dim, -1, mem);
        t2b_ancestor_exclude_nodej.init(hyparams.tree_lstm_dim, -1, mem);
	...
	inline void forward(Graph *cg, const vector<CAction> &actions, const AtomFeatures &atomFeat, PNode prevStateNode) {
        ...
        if (atomFeat.next_dist == 0) {
            ...
            for (int context = 1; context <= opt->word_context; context++) {
                ...
                p_word_tanh_conv2_context = (position >= 0 && position < atomFeat.word_size) ? &(atomFeat.p_word_tanh_conv2->at(position)) : &bucket_word_hidden2;
                states.push_back(p_word_tanh_conv2_context);
            ...
            int anc = (*atomFeat.p_common_roots)[i][atomFeat.next_dist];
