...
public:
  inline void initial(ModelParams &params, HyperParams &hyparams, AlignedMemoryPool *mem) {
      ...
      for (int idx = 0; idx < length; idx++) {
          ...
          word_tanh_conv1[idx].setParam(&params.word_tanh_conv1);
...
public:
  inline void forward(Graph* cg, const Instance& inst, HyperParams* hyparams){
      ...
      word_left_lstm.forward(cg, getPNodes(word_represents, word_size));
      word_right_lstm.forward(cg, getPNodes(word_represents, word_size));

      //FIXME:
      for (int idx = 0; idx < word_size; idx++) {
          word_tanh_conv1[idx].forward(cg, &(word_left_lstm._hiddens[idx]), &(word_right_lstm._hiddens[idx]));
      }
