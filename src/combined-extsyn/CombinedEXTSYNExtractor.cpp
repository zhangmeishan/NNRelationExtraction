#include "CombinedEXTSYNExtractor.h"
#include <chrono>
#include <omp.h>
#include <set>
...
void Extractor::train(const string &trainFile, const string &devFile, const string &testFile, const string &modelFile,
    ...
    for (int idx = 0; idx < otherInsts.size(); idx++) {
        for (int idy = 0; idy < otherInsts[idx].size(); idy++) {
            word_count += otherInsts[idx][idy].words.size();
        }
    }
    ext_count = 0;
    ...
    for (int idx = 0; idx < m_options.testFiles.size(); idx++) {
        getExternalFeats(otherInsts[idx], m_options.testFiles[idx] + ".dump");
    }
    ...
    for (int iter = 0; iter < maxIter; ++iter) {
        if (decays[iter]) {
            m_options.adaAlpha = m_options.decay * m_options.adaAlpha;
