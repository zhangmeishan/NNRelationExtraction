  ...
  public:
    inline void forward(Graph *cg, const vector<CAction> &actions, const AtomFeatures &atomFeat, PNode prevStateNode) {
        ...
        if (!atomFeat.bRel) {
            ...
            //extended syntactic features
            {
                PNode p_word_context = &(atomFeat.p_ext_word_left_lstm->_hiddens[atomFeat.ner_next_position]);
                states.push_back(p_word_context);
                p_word_context = &(atomFeat.p_ext_word_right_lstm->_hiddens[atomFeat.ner_next_position]);
                states.push_back(p_word_context);

                //entity-level
                PNode left_lstm_node_end = (atomFeat.ner_last_end >= 0) ? &(atomFeat.p_ext_word_left_lstm->_hiddens[atomFeat.ner_last_end]) : pseudo_word;
                PNode left_lstm_node_start = (atomFeat.ner_last_start > 0) ? &(atomFeat.p_ext_word_left_lstm->_hiddens[atomFeat.ner_last_start - 1]) : pseudo_word;
                ext_left_lstm_entity.forward(cg, left_lstm_node_end, left_lstm_node_start);
                states.push_back(&ext_left_lstm_entity);

                PNode right_lstm_node_end = (atomFeat.ner_last_end >= 0) ? &(atomFeat.p_ext_word_right_lstm->_hiddens[atomFeat.ner_last_end]) : pseudo_word;
                PNode right_lstm_node_start = (atomFeat.ner_last_start > 0) ? &(atomFeat.p_ext_word_right_lstm->_hiddens[atomFeat.ner_last_start - 1]) : pseudo_word;
                ext_right_lstm_entity.forward(cg, right_lstm_node_end, right_lstm_node_start);
            ...
            {
                PNode left_lstm_left = (start_i >= 1) ? &(atomFeat.p_ext_word_left_lstm->_hiddens[start_i - 1]) : pseudo_word;
                states.push_back(left_lstm_left);

                PNode left_lstm_node_i = &(atomFeat.p_ext_word_left_lstm->_hiddens[i]);
                ext_left_lstm_pointi.forward(cg, left_lstm_node_i, left_lstm_left);
                states.push_back(&ext_left_lstm_pointi);

                ext_left_lstm_middle.forward(cg, &(atomFeat.p_ext_word_left_lstm->_hiddens[start_j - 1]), left_lstm_node_i);
                states.push_back(&ext_left_lstm_middle);

                PNode left_lstm_node_j = &(atomFeat.p_ext_word_left_lstm->_hiddens[j]);
                ext_left_lstm_pointj.forward(cg, left_lstm_node_j, &(atomFeat.p_ext_word_left_lstm->_hiddens[start_j - 1]));
                states.push_back(&ext_left_lstm_pointj);

                ext_left_lstm_end.forward(cg, &(atomFeat.p_ext_word_left_lstm->_hiddens[atomFeat.word_size - 1]), left_lstm_node_j);
                states.push_back(&ext_left_lstm_end);


                PNode right_lstm_right = (j < atomFeat.word_size - 1) ? &(atomFeat.p_ext_word_right_lstm->_hiddens[j + 1]) : pseudo_word;
                states.push_back(right_lstm_right);

                PNode right_lstm_node_j = &(atomFeat.p_ext_word_right_lstm->_hiddens[j]);
                ext_right_lstm_pointj.forward(cg, right_lstm_node_j, right_lstm_right);
                states.push_back(&ext_right_lstm_pointj);

                ext_right_lstm_middle.forward(cg, &(atomFeat.p_ext_word_right_lstm->_hiddens[i + 1]), right_lstm_node_j);
                states.push_back(&ext_right_lstm_middle);

                PNode right_lstm_node_i = &(atomFeat.p_ext_word_right_lstm->_hiddens[i]);
                ext_right_lstm_pointi.forward(cg, right_lstm_node_i, &(atomFeat.p_ext_word_right_lstm->_hiddens[i + 1]));
                states.push_back(&ext_right_lstm_pointi);

                ext_right_lstm_start.forward(cg, &(atomFeat.p_ext_word_right_lstm->_hiddens[0]), right_lstm_node_i);
