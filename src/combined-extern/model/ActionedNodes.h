...
public:
	inline void initial(ModelParams &params, HyperParams &hyparams, AlignedMemoryPool *mem) {
        ...
        //extern features
        ext_left_lstm_entity.init(hyparams.ext_lstm_dim, -1, mem);
        ext_right_lstm_entity.init(hyparams.ext_lstm_dim, -1, mem);

        ext_left_lstm_middle.init(hyparams.ext_lstm_dim, -1, mem);
        ext_left_lstm_end.init(hyparams.ext_lstm_dim, -1, mem);
        ext_left_lstm_pointi.init(hyparams.ext_lstm_dim, -1, mem);
        ext_left_lstm_pointj.init(hyparams.ext_lstm_dim, -1, mem);
        ext_right_lstm_middle.init(hyparams.ext_lstm_dim, -1, mem);
        ext_right_lstm_start.init(hyparams.ext_lstm_dim, -1, mem);
        ext_right_lstm_pointi.init(hyparams.ext_lstm_dim, -1, mem);
        ext_right_lstm_pointj.init(hyparams.ext_lstm_dim, -1, mem);

        bucket_ext.init(hyparams.ext_lstm_dim, -1, mem);
        bucket_ext.set_bucket();
	}
...
public:
	inline void forward(Graph *cg, const vector<CAction> &actions, const AtomFeatures &atomFeat, PNode prevStateNode) {
        ...
        if (atomFeat.next_dist == 0) {
            ...
            left_lstm1_node_end = (atomFeat.next_i > 0) ? atomFeat.p_ext_word_left_lstm[atomFeat.next_i - 1] : &bucket_ext;
            left_lstm1_node_start = (atomFeat.last_start > 0) ? atomFeat.p_ext_word_left_lstm[atomFeat.last_start - 1] : &bucket_ext;
            ext_left_lstm_entity.forward(cg, left_lstm1_node_end, left_lstm1_node_start);
            states.push_back(&ext_left_lstm_entity);

            right_lstm1_node_end = (atomFeat.last_start >= 0) ? atomFeat.p_ext_word_right_lstm[atomFeat.last_start] : &bucket_ext;
            right_lstm1_node_start = (atomFeat.next_i > 0) ? atomFeat.p_ext_word_right_lstm[atomFeat.next_i] : &bucket_ext;
            ...
            //extern features
            PNode ext_left_lstm_left = (start_i >= 1) ? atomFeat.p_ext_word_left_lstm[start_i - 1] : &bucket_ext;
            states.push_back(ext_left_lstm_left);

            PNode ext_left_lstm_node_i = atomFeat.p_ext_word_left_lstm[i];
            ext_left_lstm_pointi.forward(cg, ext_left_lstm_node_i, ext_left_lstm_left);
            states.push_back(&ext_left_lstm_pointi);

            ext_left_lstm_middle.forward(cg, atomFeat.p_ext_word_left_lstm[start_j - 1], ext_left_lstm_node_i);
            states.push_back(&ext_left_lstm_middle);

            PNode ext_left_lstm_node_j = atomFeat.p_ext_word_left_lstm[j];
            ext_left_lstm_pointj.forward(cg, ext_left_lstm_node_j, atomFeat.p_ext_word_left_lstm[start_j - 1]);
            states.push_back(&ext_left_lstm_pointj);

            ext_left_lstm_end.forward(cg, atomFeat.p_ext_word_left_lstm[atomFeat.word_size - 1], ext_left_lstm_node_j);
            states.push_back(&ext_left_lstm_end);

            PNode ext_right_lstm_right = (j < atomFeat.word_size - 1) ? atomFeat.p_ext_word_right_lstm[j + 1] : &bucket_ext;
            states.push_back(ext_right_lstm_right);

            PNode ext_right_lstm_node_j = atomFeat.p_ext_word_right_lstm[j];
            ext_right_lstm_pointj.forward(cg, ext_right_lstm_node_j, ext_right_lstm_right);
            states.push_back(&ext_right_lstm_pointj);

            ext_right_lstm_middle.forward(cg, atomFeat.p_ext_word_right_lstm[i + 1], ext_right_lstm_node_j);
            states.push_back(&ext_right_lstm_middle);

            PNode ext_right_lstm_node_i = atomFeat.p_ext_word_right_lstm[i];
            ext_right_lstm_pointi.forward(cg, ext_right_lstm_node_i, atomFeat.p_ext_word_right_lstm[i + 1]);
            states.push_back(&ext_right_lstm_pointi);

            ext_right_lstm_start.forward(cg, atomFeat.p_ext_word_right_lstm[0], ext_right_lstm_node_i);
