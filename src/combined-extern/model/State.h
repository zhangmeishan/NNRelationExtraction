...
public:
    ...
    inline void prepare(HyperParams* hyper_params, ModelParams* model_params, GlobalNodes* global_nodes) {
        ...
        _atomFeat.p_ext_word_left_lstm.resize(_word_size);
        _atomFeat.p_ext_word_right_lstm.resize(_word_size);
        for (int idx = 0; idx < _word_size; idx++) {
            _atomFeat.p_ext_word_left_lstm[idx] = _inst->syn_feats[idx][4];
            _atomFeat.p_ext_word_right_lstm[idx] = _inst->syn_feats[idx][5];
        }
        _atomFeat.p_ext_word_tanh_conv = global_nodes == NULL ? NULL : &(global_nodes->ext_word_tanh_conv);
    }
};
