...
struct ActionedNodes {
  ...
  BucketNode bucket;
  BucketNode bucket_word_hidden2;
...
public:
  inline void initial(ModelParams &params, HyperParams &hyparams, AlignedMemoryPool *mem) {
    ...
    ner_state_hidden.init(hyparams.state_hidden_dim, hyparams.dropProb, mem);
    ...
    */
    ...
    rel_state_hidden.init(hyparams.state_hidden_dim, hyparams.dropProb, mem);
...
public:
  inline void forward(Graph *cg, const vector<CAction> &actions, const AtomFeatures &atomFeat, PNode prevStateNode) {
    ...
    bucket.forward(cg, 0);
    bucket_word_hidden2.forward(cg, 0);
    bucket_state_hidden.forward(cg, 0);
    PNode pseudo_lstm = &(bucket);
    PNode pseudo_word = &(bucket_word_hidden2);
    PNode pseudo_state = &(bucket_word_hidden2);
    ...
    if (atomFeat.next_dist == 0) {
      ...
      PNode  p_word_tanh_conv2_context = &(atomFeat.p_word_tanh_conv2->at(atomFeat.next_i));
      states.push_back(p_word_tanh_conv2_context);
      for (int context = 1; context <= opt->word_context; context++) {
        position = atomFeat.next_i + context;
        p_word_tanh_conv2_context = (position >= 0 && position < atomFeat.word_size) ? &(atomFeat.p_word_tanh_conv2->at(position)) : pseudo_word;
        states.push_back(p_word_tanh_conv2_context);
      ...
      */
      ...
      PNode left_lstm_left = (start_i >= 1) ? &(atomFeat.p_word_left_lstm->_hiddens[start_i - 1]) : pseudo_lstm;
      ...
      PNode right_lstm_right = (j < atomFeat.word_size - 1) ? &(atomFeat.p_word_right_lstm->_hiddens[j + 1]) : pseudo_lstm;
