...
struct GlobalNodes {
  ...
  public:
    inline void resize(int max_length, int max_clength) {
        ...
        word_windows.resize(max_length);
  ...
  public:
    inline void initial(ModelParams &params, HyperParams &hyparams, AlignedMemoryPool *mem) {
        ...
        word_windows.init(hyparams.word_represent_dim, hyparams.word_context, mem);
  ...
  public:
    inline void forward(Graph* cg, const Instance& inst, HyperParams* hyparams) {
        ...
        word_windows.forward(cg, getPNodes(word_represents, word_size));

        for (int idx = 0; idx < word_size; idx++) {
            word_tanh_conv1[idx].forward(cg, &(word_windows._outputs[idx]));
        }
