...
class CStateItem {
  ...
  public:
    ...
    inline void prepare(HyperParams* hyper_params, ModelParams* model_params, GlobalNodes* global_nodes) {
        ...
        _atomFeat.p_dep_b2t_lstm = global_nodes == NULL ? NULL : &(global_nodes->dep_b2t_lstm);
        _atomFeat.p_dep_t2b_lstm = global_nodes == NULL ? NULL : &(global_nodes->dep_t2b_lstm);
        _atomFeat.p_common_roots = global_nodes == NULL ? NULL : &(global_nodes->common_roots);
