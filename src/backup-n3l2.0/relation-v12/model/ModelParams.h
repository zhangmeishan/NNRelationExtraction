...
class ModelParams {
  ...
  public:
    bool initial(HyperParams &opts, AlignedMemoryPool *mem) {
        ...
        word_left_lstm.initial(opts.word_lstm_dim, opts.word_represent_dim, mem); //left lstm
        word_right_lstm.initial(opts.word_lstm_dim, opts.word_represent_dim, mem); //right lstm
        word_tanh_conv1.initial(opts.word_hidden1_dim, opts.word_lstm_dim, opts.word_lstm_dim, true, mem);
