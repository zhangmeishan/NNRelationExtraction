...
public:
  inline void initial(ModelParams &params, HyperParams &hyparams, AlignedMemoryPool *mem) {
      ...
      word_left_lstm.init(&params.word_left_lstm, 0.25, true, mem);
      word_right_lstm.init(&params.word_right_lstm, 0.25, false, mem);


      for (int idx = 0; idx < length; idx++) {
          for (int idy = 0; idy < char_inputs[idx].size(); idy++) {
              char_inputs[idx][idy].init(hyparams.char_dim, 0.15, mem);
          ...
          char_represents[idx].init(hyparams.char_hidden_dim, 0.15, mem);
          word_inputs[idx].init(hyparams.word_dim, 0.15, mem);
          word_ext_inputs[idx].init(hyparams.word_ext_dim, 0.15, mem);
          tag_inputs[idx].init(hyparams.tag_dim, 0.15, mem);
          word_represents[idx].init(hyparams.word_represent_dim, -1, mem);
          word_tanh_conv1[idx].init(hyparams.word_hidden1_dim, 0.35, mem);
          word_tanh_conv2[idx].init(hyparams.word_hidden2_dim, 0.35, mem);
      }
  }
