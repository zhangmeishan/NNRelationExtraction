...
struct ActionedNodes {
    ...
    BucketNode bucket;
    BucketNode bucket_word_hidden;
    BucketNode bucket_state_hidden;
    HyperParams *opt;
  ...
  public:
    inline void forward(Graph *cg, const vector<CAction> &actions, const AtomFeatures &atomFeat, PNode prevStateNode) {
        ...
        bucket.forward(cg, 0);
        bucket_word_hidden.forward(cg, 0);
        bucket_state_hidden.forward(cg, 0);
        PNode pseudo_lstm = &(bucket);
        PNode pseudo_word = &(bucket_word_hidden);
        PNode pseudo_state = &(bucket_state_hidden);
        ...
        if (atomFeat.next_dist == 0) {
            ...
            for (int context = 1; context <= opt->word_context; context++) {
                ...
                p_word_tanh_conv_context = (position >= 0 && position < atomFeat.word_size) ? &(atomFeat.p_word_tanh_conv2->at(position)) : pseudo_word;
                states.push_back(p_word_tanh_conv_context);
            ...
            //entity-level
            PNode left_lstm1_node_end = (atomFeat.next_i > 0) ? &(atomFeat.p_word_left_lstm1->_hiddens[atomFeat.next_i - 1]) : pseudo_lstm;
            PNode left_lstm1_node_start = (atomFeat.last_start > 0) ? &(atomFeat.p_word_left_lstm1->_hiddens[atomFeat.last_start - 1]) : pseudo_lstm;
            left_lstm1_entity.forward(cg, left_lstm1_node_end, left_lstm1_node_start);
            states.push_back(&left_lstm1_entity);

            PNode right_lstm1_node_end = (atomFeat.last_start >= 0) ? &(atomFeat.p_word_right_lstm1->_hiddens[atomFeat.last_start]) : pseudo_lstm;
            PNode right_lstm1_node_start = (atomFeat.next_i > 0) ? &(atomFeat.p_word_right_lstm1->_hiddens[atomFeat.next_i]) : pseudo_lstm;
            ...
            */
            ...
            PNode left_lstm1_left = (start_i >= 1) ? &(atomFeat.p_word_left_lstm1->_hiddens[start_i - 1]) : pseudo_lstm;
            ...
            PNode right_lstm1_right = (j < atomFeat.word_size - 1) ? &(atomFeat.p_word_right_lstm1->_hiddens[j + 1]) : pseudo_lstm;
