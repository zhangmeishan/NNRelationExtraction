...
struct GlobalNodes {
    ...
    vector<BiNode> ext_word_tanh_conv;

    LSTM1Builder ext_word_left_lstm;
    LSTM1Builder ext_word_right_lstm;
    vector<BiNode> ext_word_tanh_conv1;
  ...
  public:
    inline void resize(int max_length, int max_clength) {
        ...
        ext_word_tanh_conv.resize(max_length);
        ext_word_left_lstm.resize(max_length);
        ext_word_right_lstm.resize(max_length);
        ext_word_tanh_conv1.resize(max_length);
    }
  ...
  public:
    inline void initial(ModelParams &params, HyperParams &hyparams, AlignedMemoryPool *mem) {
        ...
        for (int idx = 0; idx < length; idx++) {
            ...
            ext_word_tanh_conv[idx].setParam(&params.ext_word_tanh_conv);
            ext_word_tanh_conv1[idx].setParam(&params.ext_word_tanh_conv1);
        }
        ...
        ext_word_left_lstm.init(&params.ext_word_left_lstm, hyparams.dropProb, true, mem);
        ext_word_right_lstm.init(&params.ext_word_right_lstm, hyparams.dropProb, false, mem);
        ...
        for (int idx = 0; idx < length; idx++) {
            ...
            word_tanh_conv1[idx].init(hyparams.word_hidden_dim, hyparams.dropProb, mem);
            word_tanh_conv2[idx].init(hyparams.word_hidden_dim, hyparams.dropProb, mem);
            ext_word_tanh_conv[idx].init(hyparams.word_hidden_dim, hyparams.dropProb, mem);
            ext_word_tanh_conv1[idx].init(hyparams.word_hidden_dim, hyparams.dropProb, mem);
        }
    }
  ...
  public:
    inline void forward(Graph* cg, const Instance& inst, HyperParams* hyparams) {
        ...
        for (int idx = 0; idx < word_size; idx++) {
            ext_word_tanh_conv[idx].forward(cg, inst.syn_feats[idx][4], inst.syn_feats[idx][5]);
        }

        ext_word_left_lstm.forward(cg, getPNodes(ext_word_tanh_conv, word_size));
        ext_word_right_lstm.forward(cg, getPNodes(ext_word_tanh_conv, word_size));

        for (int idx = 0; idx < word_size; idx++) {
            ext_word_tanh_conv1[idx].forward(cg, &(ext_word_left_lstm._hiddens[idx]), &(ext_word_right_lstm._hiddens[idx]));
        }
