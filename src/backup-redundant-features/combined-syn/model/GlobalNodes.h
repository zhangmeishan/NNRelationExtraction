...
struct GlobalNodes {
    ...
    vector<BiNode> dep_word_tanh_conv;
  ...
  public:
    inline void resize(int max_length, int max_clength) {
        ...
        dep_word_tanh_conv.resize(max_length);
    }
  ...
  public:
    inline void initial(ModelParams &params, HyperParams &hyparams, AlignedMemoryPool *mem) {
        ...
        for (int idx = 0; idx < length; idx++) {
            ...
            dep_word_tanh_conv[idx].setParam(&params.dep_word_tanh_conv);
        ...
        for (int idx = 0; idx < length; idx++) {
            ...
            dep_word_tanh_conv[idx].init(hyparams.word_hidden_dim, hyparams.dropProb, mem);
        }
    }
  ...
  public:
    inline void forward(Graph* cg, const Instance& inst, HyperParams* hyparams) {
        ...
        for (int idx = 0; idx < word_size; idx++) {
            dep_word_tanh_conv[idx].forward(cg, &(dep_b2t_lstm._hiddens[idx]), &(dep_t2b_lstm._hiddens[idx]));
        }
    }
